...
a run named `demo` looks like this:
...
Configured project-local `worktree_root`, `run_root`, and `saved_run_root` directories are added to `.git/info/exclude` automatically when they resolve inside the repository.
